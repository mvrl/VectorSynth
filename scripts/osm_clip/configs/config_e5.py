...
# params
config.batch_size = 5 # due to memory constraints
config.num_contrastive_samples = 128
config.ort_grad = True
config.lr = 1e-4
config.text_backbone = 'e5' # 'clip' or 'bert' or 'e5' or 'gritlm'
...
# logging
config.save_dir = 'checkpoints/osmclip_e5'
config.experiment_name = "osmclip_config_e5"
config.filename = f'{config.experiment_name}-{{epoch:02d}}-{{val_loss:.2f}}'

# checkpoint (best one)
config.checkpoint_path = '/data/b.j.wei/rendersynth/osm_clip/checkpoints/osmclip_e5/osmclip_config_e5-epoch=79-val_loss=3.20.ckpt'
