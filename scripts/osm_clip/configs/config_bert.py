...
config.lr = 1e-4
config.text_backbone = 'bert' # 'clip' or 'bert' or 'e5' or 'gritlm'
...
# logging
config.save_dir = 'checkpoints/osmclip_bert'
config.experiment_name = "osmclip_config_bert"
config.filename = f'{config.experiment_name}-{{epoch:02d}}-{{val_loss:.2f}}'

# checkpoint (best one)
config.checkpoint_path = '/data/b.j.wei/rendersynth/osm_clip/checkpoints/osmclip_bert-epoch=78-val_loss=3.17.ckpt'
